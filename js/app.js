...
function getClosingMessage(rating) {
    if (rating === 5) return "Tu 5/5 deja este rincón en lo más alto del archivo.";
    if (rating === 4) return "Una nota alta siempre deja ganas de seguir ampliando este recuerdo.";
    if (rating === 3) return "Queda registrada una nota equilibrada, con margen para seguir mejorándolo.";
...
function updateLoginAvailability() {
    ...
    if (locked) setFeedback(`Acceso bloqueado temporalmente. Espera ${getRemainingLockSeconds()}s.`, "error");
}

function registerVisit() {
    const visitsByProfile = JSON.parse(localStorage.getItem(VISITS_KEY) || "{}");
    const visitKey = `${activeAccess}:${activeProfileName}`;
    const visits = Number(visitsByProfile[visitKey] || 0) + 1;
    visitsByProfile[visitKey] = visits;
    localStorage.setItem(VISITS_KEY, JSON.stringify(visitsByProfile));
    visitCount.textContent = String(visits);
}

function renderFeatured(items) {
    featuredGrid.innerHTML = items.map((item) => `<article class="media-card"><figure class="media-frame"><video controls preload="metadata" playsinline src="${escapeHtml(item.file)}"></video></figure><div class="media-body"><p class="media-meta">Video</p><h4>${escapeHtml(item.title)}</h4><p class="media-description">${escapeHtml(item.description || "")}</p></div></article>`).join("");
}

function renderPhotos(items) {
    photoGrid.innerHTML = items.map((item, index) => `<article class="photo-card" data-index="${index}" tabindex="0" role="button" aria-label="Abrir ${escapeHtml(item.title)}"><figure><img src="${escapeHtml(item.file)}" alt="${escapeHtml(item.title)}"><figcaption class="photo-caption">${escapeHtml(item.title)}</figcaption></figure></article>`).join("");
}
...
function renderDailyMemory(profile) {
    const pool = [...(profile.photos || []).map((item) => ({ kind: "Foto", title: item.title, description: item.description })), ...(profile.letters || []).map((item) => ({ kind: "Carta", title: item.title, description: item.tag || item.signature || "" })), ...(profile.featured || []).map((item) => ({ kind: "Video", title: item.title, description: item.description }))];
    if (!pool.length) { dailyMemoryCard.innerHTML = "<p class=\"daily-memory-empty\">Todavia no hay recuerdos suficientes para elegir uno aleatorio.</p>"; return; }
    const selected = pool[hashString(`${activeAccess}-${activeProfileName}-${getTodaySeed()}`) % pool.length];
    dailyMemoryCard.innerHTML = `<p class="daily-memory-kind">${escapeHtml(selected.kind)}</p><h4>${escapeHtml(selected.title || "Recuerdo del dia")}</h4><p class="daily-memory-copy">${escapeHtml(selected.description || "El archivo ha querido destacar esta pieza hoy.")}</p>`;
}
...
function updateMusicUi() {
    ...
    if (!tracks.length || !activeTrack) { musicStatus.textContent = "Sin pistas"; toggleMusicButton.disabled = true; nextTrackButton.disabled = true; return; }
...
function showProfileStep() { loginForm.classList.add("hidden"); profileStep.classList.remove("hidden"); setFeedback(""); }
function showPasswordStep() { profileStep.classList.add("hidden"); loginForm.classList.remove("hidden"); introScreen.classList.add("hidden"); app.classList.add("hidden"); }
function showIntroScreen() { loginShell.classList.add("hidden"); introScreen.classList.remove("hidden"); app.classList.add("hidden"); }
function showApp() { loginShell.classList.add("hidden"); introScreen.classList.add("hidden"); app.classList.remove("hidden"); }
...
function applyProfile(profileName) {
    ...
    const visitsByProfile = JSON.parse(localStorage.getItem(VISITS_KEY) || "{}");
    visitCount.textContent = String(visitsByProfile[`${activeAccess}:${profileName}`] || 0);
    lastSurpriseIndex = -1;
    setTrack(0);
...
function unlockApp() {
    ...
    localStorage.setItem(ACCESS_KEY, activeAccess);
    registerVisit();
    playBackgroundMusic();
}
...
function handleLogin(password) {
    if (isLocked()) { updateLoginAvailability(); return; }
    const matchedAccess = Object.keys(PASSWORDS).find((key) => PASSWORDS[key] && password === PASSWORDS[key]);
    if (matchedAccess) {
        activeAccess = matchedAccess;
        localStorage.removeItem(ATTEMPTS_KEY);
        localStorage.removeItem(LOCK_KEY);
        setFeedback(matchedAccess === "future" ? "Acceso 2027 desbloqueado. Ahora elige tu nombre." : "Contraseña correcta. Ahora elige tu nombre.", "success");
        showProfileStep();
        return;
    }
    const attempts = Number(localStorage.getItem(ATTEMPTS_KEY) || 0) + 1;
    localStorage.setItem(ATTEMPTS_KEY, String(attempts));
    if (attempts >= MAX_ATTEMPTS) { const lockUntil = Date.now() + LOCK_MINUTES * 60 * 1000; localStorage.setItem(LOCK_KEY, String(lockUntil)); localStorage.removeItem(ATTEMPTS_KEY); updateLoginAvailability(); return; }
    setFeedback(`Contraseña incorrecta. Quedan ${MAX_ATTEMPTS - attempts} intento(s).`, "error");
}
